...
namespace CuscLibrary.DataTypes.Extensions
{
    public static class StringExtension
    {
        ...
        {
            return value.Mid(startIndex, value.Length - startIndex);
        }
        ...
        public static string Mid(this string value, int startIndex, int length)
        {
            ...
            if (startIndex < 0)
            {
                // Cho startIndex bắt đầu từ đầu chuỗi
                startIndex = 0;
            }
            else if (startIndex > value.Length - 1) // Nếu startIndex vượt quá chiều dài chuỗi
            {
                return String.Empty;
            }
        ...
        /// </example>
        /// 
        public static string DecorateWithUnicodeCharacter(this string value, string input, int count)
        {
            return value.DecorateWithUnicodeCharacter(input, input, count);
        }
        ...
        /// <param name="count">Số lần xuất hiện ký tự unicode.</param>
        /// <returns>Chuỗi đã được trang trí.</returns>
        public static string DecorateWithUnicodeCharacter(this string value, string prefix, string subfix, int count)
        {
            int prefixCode = Convert.ToInt32(prefix, 16);
            int subfixCode = Convert.ToInt32(subfix, 16);
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < count; i++)
            {
                sb.Append((char)prefixCode);
            }
            sb.AppendFormat(" {0} ", value);
            for (int i = 0; i < count; i++)
            {
                sb.Append((char)subfixCode);
            }
            return sb.ToString();
        ...
        {
            return value.ToTitleCase(false);
        }
        ...
        /// <returns>Chuỗi đã được chuyển thành hoa các ký tự đầu của mỗi từ trong chuỗi.</returns>
        public static string ToTitleCase(this string value, bool onlyFirstWord)
        {
            ...
            {
                return CultureInfo.CurrentCulture.TextInfo.ToTitleCase(temp);
            }
            else
            {
                string result = String.Empty;
                ...
                // Lấy ký tự đầu tiên chuyển thành Hoa
                StringBuilder sb = new StringBuilder();
                sb.Append(temp.Left(1).ToUpper());
                sb.Append(temp.Mid(1));
                result = sb.ToString();
